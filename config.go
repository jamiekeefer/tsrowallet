...
const (
	defaultCAFilename          = "thesauro.cert"
	defaultConfigFilename      = "tsrowallet.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "tsrowallet.log"
...
var (
	thesauroDefaultCAFile  = filepath.Join(dcrutil.AppDataDir("thesauro", false), "rpc.cert")
	defaultAppDataDir  = dcrutil.AppDataDir("tsrowallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of thesauro RPC server to connect to"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with thesauro"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	DcrdUsername     string                  `long:"thesaurousername" description:"Username for thesauro authentication"`
	DcrdPassword     string                  `long:"thesauropassword" default-mask:"-" description:"Password for thesauro authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy JSON-RPC and thesauro authentication (if thesaurousername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy JSON-RPC and thesauro authentication (if thesauropassword is unset)"`
...
//
// The above results in tsrowallet functioning properly without any config
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local thesauro cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local thesauro and switch to its RPC cert if it exists.
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					thesauroCertExists, err := cfgutil.FileExists(
						thesauroDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return loadConfigError(err)
					}
					if thesauroCertExists {
						cfg.CAFile.Value = thesauroDefaultCAFile
					}
				}
			}
		}
	...
	// If the thesauro username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for thesauro and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for thesauro and wallet.
	if cfg.DcrdUsername == "" {
